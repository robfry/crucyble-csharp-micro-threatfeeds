...
{
  class FeedsAlientVault
  {
    public  static AlienVaultReturnValues AlienVaultIP(List<string> sDstIP)
    {
      ...
      foreach (var ip in sDstIP)
      {
        foreach (var sLoadFeedAry in from sLoadedFeed in lLoadedFeed where sLoadedFeed.Contains(ip) select sLoadedFeed.Split('#'))
        {
