...
namespace FIDO.Threatfeeds.FIDO.Support.Threat.Feeds
{
  public class FeedsVirusTotal
  {
    ...
    public FeedsVirusTotal(string apiKey)
    {
    ...
    public static List<IPReport> VirusTotalIP(List<String> lIP)
    {
    ...
    public static List<UrlReport> VirusTotalUrl(List<string> lURL)
    {
      ...
      if (lVTURLRet == null) return null;
      if (lVTURLRet.Any()) return lVTURLRet;
      //return lVTURLRet.Count != 0 && lVTURLRet != null ? lVTURLRet : null;
      return null;
    }
    ...
    public static List<FileReport> ParseHash(string[] sMD5Hash)
    {
      ...
      var sVTKey = GetVtKey();
      var vtLogin = new Support.VirusTotal.NET.VirusTotal(sVTKey);
      var sVirusTotalHash = new List<FileReport>();
      //var fidoDB = new SqLiteDB();
      //var isPaidFeed = new SqLiteDB().ExecuteBool("Select paid_feed from configs_threatfeed_virustotal");
      ...
      {
        FidoEventHandler.SendEmail("Fido Error", "Fido Failed: {0} Exception caught in VT Hash area:" + e);
      }
      return sVirusTotalHash;
    }
    ...
    private static List<UrlReport> ParseUrl(IEnumerable<string> sURL)
    {
      ...
      //var sAcek = xfidoconf.getVarSet("securityfeed").getVarSet("virustotal").getString("acek", null);
      var sVTKey = GetVtKey();
        //new SqLiteDB().ExecuteScalar(@"select apikey from configs_threatfeed_virustotal");
      ...
      try
      {
        if (sURL != null)
        {
          ...
          {
            if (string.IsNullOrEmpty(url[i])) continue;
      ...
      catch (Exception e)
      {
        ...
        FidoEventHandler.SendEmail("Fido Error", "Fido Failed: {0} Exception caught in VT URL area:" + e);
      }
      return sVirusTotalUrl;
    }

    private static List<IPReport> ParseIP(string[] sIP)
    {
      ...
      List<IPReport> sVirusTotalIP = null;
    ...
    private static List<IPReport> GetIPReport(string[] ip, string _apiKey)
    {
      if (ip.Length <= 0)
      ...
      return GetResults<List<IPReport>>(request, true);
    }
    ...
    private static string GetVtKey()
    {
      var query = APIEndpoints.PrimaryConfig.host + APIEndpoints.PrimaryConfig.fido_configs_threatfeeds.threatfeed_api.vendor + "?key=\"virustotal\"";
      var alertRequest = (HttpWebRequest)WebRequest.Create(query);
      var stringreturn = string.Empty;
      var cdbReturn = new VtObject();
      var getREST = new RestConnection();
      stringreturn = getREST.RestCall(alertRequest);
      cdbReturn = JsonConvert.DeserializeObject<VtObject>(stringreturn);
      return cdbReturn.rows[0].value.apikey;
    }
